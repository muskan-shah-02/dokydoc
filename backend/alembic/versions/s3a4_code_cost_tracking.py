"""Add cost tracking columns to code_components

Revision ID: s3a4_code_cost
Revises: s3a3_fix_repo_id
Create Date: 2026-02-11 22:00:00.000000

Adds ai_cost_inr, token_count_input, token_count_output, and cost_breakdown
to code_components table, matching the document model's cost tracking pattern.
"""
...
revision: str = 's3a4_code_cost'
down_revision: Union[str, None] = 's3a3_fix_repo_id'
...
def upgrade() -> None:
    ...
    for col_name, col_type, default in [
        ...
        if not result.scalar():
            op.add_column('code_components', sa.Column(col_name, col_type, nullable=False, server_default=default))

    # JSONB column (nullable, no default needed)
    result = conn.execute(sa.text(
